...
from .forms import  EditProfileForm, PersonalEditForm,BlogEditForm
...
def register(request):
    if request.method == 'POST':
        ...
        if len(pass1)<8:
            return render(request,'myapp/password_validation.html')
        elif not(fn.isalpha() and ln.isalpha() and un.isalpha()):
            messages.error(request,'firstname,lastname and username should not contain numbers or special characters and no space allowed between username')
...
def dashboard(request):
    ...
    if request.method == 'GET':
        ...
        if len(pro)!=0:
            ...
            pro4 = Blog.objects.filter(category='Other',user_id=request.user.id).count()
            pro5 = Blog.objects.filter(category='Technical',user_id=request.user.id).count()
            context['pro'] = pro
            context['pro1'] = pro1
            context['pro2'] = pro2
            context['pro3'] = pro3
...
def blog(request):
    if request.method=='POST':
        ...
        if category == 'None':
            return render(request,'myapp/select_category.html')
        else:    
            title = request.POST['title']
            blog = request.POST['blog']
            bl = Blog(category=category,title=title,blog=blog,user_id=request.user)
            bl.save()
            messages.success(request,"Blog has been submitted successfully and will be approved by admin within 24 hours.")
...
def delete_post(request,id):
    ...
    return render(request,'myapp/del_post.html')

def update_post(request,id):
    data = Blog.objects.get(id=id)
    blog_data = BlogEditForm(instance=data)
    if request.method == 'POST':
        form = BlogEditForm(request.POST,instance=data)
        if form.is_valid():
            form.save()
            return render(request,'myapp/Edit_successfull.html')
    return render(request,'myapp/Edit_blog.html',{'blog_data':blog_data})
